...
WebServer server(80);
String nmeaLog = "";
const int MAX_LOG_LINES = 400;  // maksymalna liczba linii w logu
int logLineCount = 0;
...
void printWrappedLine(const char *text, uint16_t color)
{
  ...
  while (pos < len)
  {
    ...
    tft.print(line);
    cursorY += LINE_HEIGHT;
    if (cursorY >= SCREEN_HEIGHT - LINE_HEIGHT - 20)  // miejsce na diagnostykę
    {
...
function updateLog(){
    fetch('/log?t=' + Date.now()).then(resp=>resp.text()).then(txt=>{
        const logDiv = document.getElementById('log');
        logDiv.textContent = txt;
        logDiv.scrollTop = logDiv.scrollHeight;
    });
}
setInterval(updateLog, 200);  // co 1 sekundę – mniej obciążenia
updateLog();
...
void setup()
{
  ...
  esp_task_wdt_init(10, true);  // watchdog 10s
